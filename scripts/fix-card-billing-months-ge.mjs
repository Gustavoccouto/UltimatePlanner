...
function extractInstallmentNumber(transaction) {
  const metadata = transaction.metadata && typeof transaction.metadata === "object" ? transaction.metadata : {};
  return Math.max(1, Number(metadata.installment_number || transaction.installment_number || transaction.installment_current || 1));
}
...
async function recalculateInvoice(ownerId, card, billingMonth) {
  ...
  const validTransactions = (transactions || []).filter((transaction) => !transaction.is_deleted && transaction.status !== "canceled");
...
async function main() {
  let cardsQuery = supabase
    .from("credit_cards")
    .select("id,owner_id,name,closing_day,due_day,is_deleted")
    .eq("is_deleted", false);
  ...
  for (const card of cards || []) {
    ...
    for (const transaction of transactions || []) {
      ...
      console.log(`${dryRun ? "[DRY-RUN] " : ""}${card.name || card.id}: ${transaction.description || transaction.id} | ${transaction.date} | ${previousMonth || "sem mês"} -> ${nextMonth}`);
