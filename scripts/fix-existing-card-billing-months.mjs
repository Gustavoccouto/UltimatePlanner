...
function clampDay(year, monthIndex, day) {
  return Math.min(Number(day || 1), new Date(Date.UTC(year, monthIndex + 1, 0)).getUTCDate());
}
...
function calculateInstallmentBillingMonth(transaction, card) {
  const metadata = transaction.metadata && typeof transaction.metadata === "object" ? transaction.metadata : {};
  const installmentNumber = Math.max(1, Number(metadata.installment_number || 1));
  const originalPurchaseDate = typeof metadata.purchase_date === "string" ? metadata.purchase_date : null;
  ...
  return calculateBillingMonth(transaction.date, card.closing_day);
}
...
async function maybeThrow(response, context) {
  if (response.error) {
...
async function recalculateInvoice(ownerId, card, billingMonth) {
  ...
  const transactions = await maybeThrow(
    await supabase
      .from("transactions")
      .select("amount")
      .eq("owner_id", ownerId)
  ...
  const totalAmount = money((transactions || []).reduce((sum, transaction) => sum + money(transaction.amount), 0));
...
async function main() {
  let cardsQuery = supabase
    .from("credit_cards")
    .select("id,owner_id,name,closing_day,due_day")
    .eq("is_deleted", false);

  if (onlyOwnerId) cardsQuery = cardsQuery.eq("owner_id", onlyOwnerId);

  const cards = await maybeThrow(await cardsQuery, "buscar cartões");
  ...
  for (const card of cards || []) {
    ...
    const transactions = await maybeThrow(
      await supabase
        .from("transactions")
        .select("id,owner_id,date,billing_month,installment_id,installment_plan_id,metadata")
    ...
    for (const transaction of transactions || []) {
      ...
      const nextMonth = calculateInstallmentBillingMonth(transaction, card);
      ...
      if (transaction.installment_id) {
        await maybeThrow(
          ...
          `atualizar parcela ${transaction.installment_id}`
        );

        changedInstallments += 1;
      }
    }

    const plans = await maybeThrow(
      await supabase
        .from("installment_plans")
        .select("id,owner_id,first_date,metadata")
        .eq("owner_id", card.owner_id)
        .eq("credit_card_id", card.id)
        .eq("payment_method", "credit_card")
        .neq("status", "canceled"),
      `buscar planos do cartão ${card.name || card.id}`
    );

    for (const plan of plans || []) {
      if (!plan.first_date) continue;

      const invoiceMonth = calculateBillingMonth(plan.first_date, card.closing_day);
      const metadata = plan.metadata && typeof plan.metadata === "object" && !Array.isArray(plan.metadata)
        ? plan.metadata
        : {};

      if (metadata.invoice_month === invoiceMonth) continue;

      await maybeThrow(
        await supabase
          .from("installment_plans")
          .update({ metadata: { ...metadata, invoice_month: invoiceMonth } })
          .eq("owner_id", card.owner_id)
          .eq("id", plan.id),
        `atualizar metadata do plano ${plan.id}`
      );
  ...
  console.log(`Planos atualizados: ${changedPlans}`);
