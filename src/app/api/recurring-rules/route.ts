...
import {
  assertRecurringCardLimitAvailable,
  cleanupFutureRecurringTransactions,
...
function validateRule(payload: RulePayload) {
  ...
  if (targetType === "card" && !payload.credit_card_id) {
    throw new Error("Selecione um cartão para o gasto recorrente.");
  }

  if (targetType === "account" && !payload.account_id) {
    throw new Error("Selecione uma conta para a recorrência.");
  }
}

function buildRecord(ownerId: string, payload: RulePayload, categoryId: string | null) {
  const targetType = payload.rule_type === "recurring_income" ? "account" : payload.target_type;
...
async function resolveCategoryId(context: Exclude<Awaited<ReturnType<typeof getApiContext>>, { error: Response }>, payload: RulePayload) {
  return (
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const categoryId = await resolveCategoryId(context, payload);
    const record = buildRecord(context.user.id, payload, categoryId);

    await assertRecurringCardLimitAvailable(context.supabase, context.user.id, record);

    const { data, error } = await context.supabase.from("recurring_rules").insert(record).select("*").single();
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    await assertRecurringCardLimitAvailable(context.supabase, context.user.id, record, {
      excludeRecurringRuleId: payload.id
    });
