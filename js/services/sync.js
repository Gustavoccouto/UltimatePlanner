...
const REMOTE_SYNCABLE = ENTITY_STORES.filter((name) => !['syncQueue', 'syncErrors', 'meta'].includes(name));
const REMOTE_BATCH_SIZE = 1;

async function logSyncError(entity, recordId, message) {
  await putOne('syncErrors', {
...
export async function enqueueSync(storeName, recordId, action = 'upsert') {
  if (!recordId) {
    await logSyncError(storeName, null, `Tentativa de enfileirar sincronização sem id em ${storeName}.`);
...
export async function processSyncQueue() {
  ...
  for (const [entity, items] of Object.entries(grouped)) {
    ...
    const recordMap = new Map(records.filter(isValidStoreRecord).map((r) => [r.id, r]));
    ...
    for (let i = 0; i < items.length; i += REMOTE_BATCH_SIZE) {
      ...
      for (const item of chunk) {
        ...
        if (!record || !record.id) {
          await logSyncError(item.entity, item.recordId, `Registro ausente ou inválido para sincronização em ${item.entity}.`);
          doneWithoutSync.push({ ...item, syncStatus: 'done', updatedAt: nowIso() });
          continue;
        }
      ...
      try {
        ...
        await bulkPut(entity, validPayload.map((record) => ({ ...record, syncStatus: 'synced', syncUpdatedAt: nowIso() })), { skipInvalid: true });
        await bulkPut('syncQueue', chunk.map((item) => ({ ...item, syncStatus: 'done', updatedAt: nowIso() })), { skipInvalid: true });
        ...
        await bulkPut('syncQueue', chunk.map((item) => ({ ...item, syncStatus: 'failed', updatedAt: nowIso() })), { skipInvalid: true });
      }
    }
  }
...
function isRemoteNewer(localRecord, remoteRecord) {
  const localTime = new Date(localRecord?.updatedAt || localRecord?.syncUpdatedAt || 0).getTime();
  const remoteTime = new Date(remoteRecord?.updatedAt || remoteRecord?.syncUpdatedAt || 0).getTime();
...
export async function pullRemoteIntoLocal() {
  ...
  for (const storeName of REMOTE_SYNCABLE) {
    ...
    const merged = new Map(localRecords.filter(isValidStoreRecord).map((record) => [record.id, record]));

    for (const remoteRecord of remoteRecords) {
      if (!remoteRecord || !remoteRecord.id) {
        await logSyncError(storeName, null, `Registro remoto inválido ignorado em ${storeName}.`);
        continue;
      }
...
export async function requeueFailedSync() {
  ...
  await bulkPut('syncQueue', failed.map((item) => ({ ...item, syncStatus: 'pending', updatedAt: nowIso() })), { skipInvalid: true });
