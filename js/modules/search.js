...
const SEARCH_GROUPS = [
  {
    ...
    renderMeta: (item) => `${item.bankName || 'Banco'} • ${item.type || 'Conta'}`,
  },
  {
    title: 'Transações',
    route: 'transactions',
    ...
    renderMeta: (item) => `${item.category || 'Sem categoria'} • ${datePt(item.date)}`,
  ...
  },
  {
    ...
    renderMeta: (item) => `${item.brand || 'Bandeira'} • venc. ${item.dueDay || '-'} / fecha ${item.closingDay || '-'}`,
  ...
  },
  {
    ...
    renderMeta: (item) => `${item.status || 'Em andamento'} • prazo ${item.deadline ? datePt(item.deadline) : 'aberto'}`,
  ...
  },
  {
    ...
    renderMeta: (item) => `${item.category || 'Meta'} • prazo ${item.targetDate ? datePt(item.targetDate) : 'aberto'}`,
    renderValue: (item) => percent(Math.min(100, ((Number(item.currentAmount || 0) / Math.max(Number(item.targetAmount || 1), 1)) * 100))),
  },
  {
    title: 'Investimentos',
    route: 'investments',
    ...
    renderMeta: (item) => `${item.type || 'Investimento'} • ${item.broker || 'Sem corretora'}`,
    renderValue: (item) => currency(item.currentValue || item.amountInvested || 0),
  }
];
...
export function renderSearch() {
  ...
  const groups = SEARCH_GROUPS
    .map((group) => ({
      ...group,
      items: group.getItems().filter((item) => includesAny(group.matcher(item), query)).slice(0, 6)
    }))
