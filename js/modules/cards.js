...
export function renderCards() {
  ...
  return `
    ...
    <section class="module-stack">
      ...
      <section class="card p-4 md:p-6 overflow-hidden">
        <div class="section-head section-head-spaced"><div><div class="text-sm text-slate-500">Parcelamentos</div><div class="section-title">Planos ativos</div></div><span class="badge badge-muted">${installmentPlans.length} plano(s)</span></div>
        <div class="overflow-auto">
          <table class="data-table">
            <thead><tr><th>Descrição</th><th>Cartão</th><th>Total</th><th>Progresso</th><th>Compra</th><th>Ações</th></tr></thead>
            <tbody>
              ${installmentPlans.map(renderInstallmentPlanRows).join("") || `<tr><td colspan="6" class="text-center text-slate-500 py-10">Nenhum parcelamento ativo.</td></tr>`}
            </tbody>
          </table>
        </div>
      </section>
  ...
  try {
    ...
    const transactions = Array.from({ length: installmentCount }).map(
      (_, index) => {
        ...
        consumed += amount;
        const installmentDate = addMonths(payload.purchaseDate, index);
        return {
          id: createId("tx"),
          ...
          billingMonth: addMonthsToMonthKey(
            getCardBillingMonth(
...
async function updateInstallmentStatus(transactionId, nextStatus) {
  try {
    ...
    const plan = await getOne(
      "installmentPlans",
      updatedTransaction.installmentPlanId,
    );
    if (plan && !plan.isDeleted) {
      const related = state.data.transactions
        .filter(
          (transaction) =>
            !transaction.isDeleted &&
      ...
      const remainingInstallments = related.filter(
        (transaction) =>
...
function renderInstallmentPlanRows(plan) {
  return `
    <tr>
      <td>
      ...
      <td>${plan.card?.name || "Cartão removido"}</td>
                ...
                (installment) => `
                  <div class="rounded-[18px] border border-slate-200 bg-white p-3 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
                    <div>
                      ...
                      <div class="text-sm text-slate-500">${datePt(installment.date)} • ${currency(installment.amount)} • Fatura ${installment.billingMonth || "—"}</div>
                    </div>
