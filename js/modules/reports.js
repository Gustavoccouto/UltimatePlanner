...
export function renderReports() {
  const activeTransactions = getTransactionsForMonth(
  ...
  const categoryTotals = aggregate(
    activeTransactions.filter((item) =>
      ["expense", "card_expense"].includes(item.type),
    ),
    "category",
  );
  ...
  const projectCosts = activeTransactions
    ...
    .reduce((acc, item) => {
      acc[item.projectId] =
        (acc[item.projectId] || 0) + Number(item.amount || 0);
      return acc;
    }, {});
  ...
  const expense = activeTransactions
    .filter((item) => ["expense", "card_expense"].includes(item.type))
    .reduce((sum, item) => sum + Number(item.amount || 0), 0);
        ...
        <article class="card p-6 overflow-hidden">
          ...
          ${
            Object.keys(accountTotals).length
              ? `<div class="overflow-auto"><table class="data-table"><thead><tr><th>Conta</th><th>Total movimentado</th></tr></thead><tbody>${Object.entries(
                  accountTotals,
                )
                  .sort((a, b) => b[1] - a[1])
                  .map(
                    ([name, total]) =>
                      `<tr><td class="font-semibold">${name}</td><td>${currency(total)}</td></tr>`,
                  )
                  .join("")}</tbody></table></div>`
              : `<div class="text-slate-500">Ainda não há movimentações suficientes para ${monthLabel(state.ui.selectedMonth)}.</div>`
          }
        </article>
        <article class="card p-6 overflow-hidden">
...
export function mountReportCharts() {
  ...
  const flowPoints = monthlyFlow(
    state.data.transactions,
    state.ui.selectedMonth,
    state.data.creditCards,
  ).slice(-8);
  const expenseByCategory = aggregate(
  ...
  reportChartOne = new Chart(flowCanvas.getContext("2d"), {
    ...
    data: {
      ...
      datasets: [
        {
          label: "Receitas",
          data: flowPoints.map((item) => item.income),
          tension: 0.35,
          borderWidth: 3,
          pointRadius: 3,
          borderColor: "#38bdf8",
        ...
        {
          label: "Despesas",
          data: flowPoints.map((item) => item.expense),
          tension: 0.35,
          borderWidth: 3,
          pointRadius: 3,
          borderColor: "#fb7185",
          backgroundColor: "rgba(251,113,133,.14)",
          fill: true,
        },
      ],
    },
