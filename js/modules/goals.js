  ...
  document
    ...
    .forEach((button) =>
      button.addEventListener("click", () =>
        openGoalModal(button.dataset.goalEdit),
      ),
    );
  document
    ...
    .forEach((button) =>
      button.addEventListener("click", () =>
        confirmGoalDelete(button.dataset.goalDelete),
      ),
    );
}
...
async function saveGoal(event) {
  ...
  try {
    ...
    closeModal();
    toast(
      existing ? "Meta atualizada com sucesso." : "Meta salva com sucesso.",
      "success",
    );
  } catch (error) {
...
function confirmGoalDelete(goalId) {
  confirmDialog({
    title: "Excluir meta",
    message:
      "A meta será marcada como excluída, preservando histórico e sincronização.",
