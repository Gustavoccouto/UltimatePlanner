...
export function renderInvestments() {
  ...
  const totalInvested = investments.reduce((sum, item) => sum + Number(item.amountInvested || 0), 0);
  const currentValue = investments.reduce((sum, item) => sum + Number(item.currentValue || item.amountInvested || 0), 0);
  const profitability = totalInvested ? ((currentValue - totalInvested) / totalInvested) * 100 : 0;
...
export function bindInvestmentsEvents() {
  document.getElementById('new-investment-btn')?.addEventListener('click', () => openInvestmentModal());
  document.querySelectorAll('[data-investment-edit]').forEach((button) => button.addEventListener('click', () => openInvestmentModal(button.dataset.investmentEdit)));
  document.querySelectorAll('[data-investment-delete]').forEach((button) => button.addEventListener('click', () => confirmDelete(button.dataset.investmentDelete)));
}
...
async function openInvestmentModal(investmentId = null) {
  const existing = investmentId ? await getOne('investments', investmentId) : null;
  openModal(`
...
async function saveInvestment(event) {
  ...
  try {
    ...
    await loadState();
    closeModal();
    toast(existing ? 'Investimento atualizado com sucesso.' : 'Investimento salvo com sucesso.', 'success');
  } catch (error) {
...
function confirmDelete(investmentId) {
  confirmDialog({
    ...
    message: 'O investimento será marcado como excluído, preservando histórico e sincronização.',
