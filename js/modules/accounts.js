...
export function renderAccounts() {
  ...
  const activeTransactions = state.data.transactions.filter(
    (item) => !item.isDeleted,
  );
    ...
    <section class="module-stack">
      <div class="module-card-grid">
        ${
          accounts
            .map((account) => {
              const txCount = activeTransactions.filter(
                (tx) =>
          ...
          emptyState(
            ...
            "Adicione sua primeira conta para começar a derivar saldos e organizar movimentações.",
          )
        }
      </div>
            ...
            <tbody>
              ${
                ...
                <tr>
                  ...
                  .join("") ||
                `<tr><td colspan="5" class="text-center text-slate-500 py-10">Nenhuma conta cadastrada.</td></tr>`
              }
            </tbody>
          </table>
        </div>
...
export function bindAccountsEvents() {
  document
    .getElementById("new-account-btn")
  ...
  document.querySelectorAll("[data-account-delete]").forEach((button) => {
    button.addEventListener("click", () =>
      confirmDelete(button.dataset.accountDelete),
    );
  });
}
...
async function saveAccount(event) {
  ...
  try {
    ...
    closeModal();
    toast(
      existing ? "Conta atualizada com sucesso." : "Conta salva com sucesso.",
      "success",
    );
  } catch (error) {
